...
public class Main {
  ...
  public static final int POOL_SIZE = 4;
	...
	public static void main(String args[]) throws IOException {
        ...
        Thread [] threadPool = new Thread[Main.POOL_SIZE];

        for(int i = 0; i < filesList.size(); i++) {
          SearchTask task = new SearchTask(new SearchJob(filesList.get(i)), pattern);
          int targetThread = i % Main.POOL_SIZE;
          if (threadPool[targetThread] != null) {
            try {
              threadPool[targetThread].join();
            } catch(InterruptedException e) { }
          }

          threadPool[targetThread] = new Thread(threadGroup, task, "Thread: " + targetThread);
          threadPool[targetThread].start();
          System.out.println("Running task on Thread: " + targetThread);
        }
			}
			else {
